...
import React, { useState, useRef, useEffect as useReactEffect, useContext } from 'react';
...
export default function Record() {
  ...
  // Load notes based on instrument
  useReactEffect(() => {
    (async () => {
    ...
    const noteNames = ['A','Ab','B','Bb','C','D','E','Eb','G','Gb']; // <--- add this line
    const noteArray = noteNames.map((name, i) => ({
      name,
      file: files[i] // already the require() object
    }));
      ...
      recordedNotes.forEach(({ noteName, timestamp }) => {
        ...
        if (delay >= 0) {
          const timer = setTimeout(() => {
            ...
            if (noteObj) playAudio(noteObj.file);
          }, delay);
    ...
    <View style={styles.container}>
      {pianoKeyOrder.map(i => (
        <Pressable
          ...
          onPress={() => handleNotePress(notes[i])}
        />
      ))}
